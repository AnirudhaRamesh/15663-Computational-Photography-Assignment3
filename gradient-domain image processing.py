...
sigma = 40
tau = 0.7
fused_image_r_div = fuse_gradient_field(museum_amb_norm[:,:,0], museum_flash_norm[:,:,0], sigma, tau)
fused_image_g_div = fuse_gradient_field(museum_amb_norm[:,:,1], museum_flash_norm[:,:,1], sigma, tau)
fused_image_b_div = fuse_gradient_field(museum_amb_norm[:,:,2], museum_flash_norm[:,:,2], sigma, tau)

I_star_r = poisson_solver(fused_image_r_div, museum_amb_norm[:,:,0], N=5000, eps=0.0001)
I_star_g = poisson_solver(fused_image_g_div, museum_amb_norm[:,:,1], N=5000, eps=0.0001)
I_star_b = poisson_solver(fused_image_b_div, museum_amb_norm[:,:,2], N=5000, eps=0.0001)
...
plt.imsave('outputs/outputs_report_part2/boundary_condition_variation/flashremoved_sigma{}_tau{}_boundary_ambflashmean.png'.format(sigma,tau),np.clip(I_star,0,1))
plt.imsave('outputs/outputs_report_part2/boundary_condition_variation/flashremoved_sigma{}_tau{}_scaled125percent_boundary_ambflashmean.png'.format(sigma,tau),np.clip(I_star*1.25,0,1))
